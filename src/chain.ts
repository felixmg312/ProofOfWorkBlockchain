...
const TxPK = '3f0bc71a375b574e4bda3ddf502fe1afd99aa020bf6049adfe525d9ad18ff33f'
class ChainManager {
  ...
  async onValidBlockArrival(block: Block) {
    ...
    if (height > this.longestChainHeight) {
      ...
      await this.save()
      let txToInclude = await mempool.findTxSigned(TxPK)
      await workerManager.worker.terminate()
      workerManager.init('./worker.ts',block,txToInclude)
    }
  }
}
