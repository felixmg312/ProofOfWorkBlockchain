...
class Block {
    ...
    updateNounce() {
        if (!this.hasPoW()) {
            let nonce = BigInt(`0x${this.nonce}`) + BigInt(1);
            let nonce_str = (0, bigint_conversion_1.bigintToHex)(nonce, false, 32);
            if (nonce_str.length < 64) {
                this.nonce = nonce_str.padStart(64, '0');
            }
            this.blockid = (0, hash_1.hash)((0, json_canonicalize_1.canonicalize)(this.toNetworkObject()));
        }
    }
